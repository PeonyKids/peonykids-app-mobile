...
class MainState extends ChangeNotifier {
  ...
  Future<void> signUpUser(BuildContext context) async {
    ...
    if (response.statusCode == 200) {
     ...
     SignInUser(false);
      success == 'true' ?  token = registrationData['jwtToken'] : null;
    }
    notifyListeners();
  }
  ...
  Future<void> resendOtpAgain(BuildContext context) async {
    final url = Uri.parse(
  ...
  Future<void> verifyEmail() async {
    final url = Uri.parse(
        'https://seahorse-app-9ubkt.ondigitalocean.app/api/v1/auth/verify-email/$token');
    print('hi');
    final response = await post(
      url,
      headers: {
